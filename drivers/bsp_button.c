...
*/
static uint8_t IsKeyDownUser(void)
{
    if (GPIO_ReadInputDataBit(GPIOG, GPIO_Pin_8) == Bit_SET) return 0;
    return 1;
}
static uint8_t IsKeyDownTamper(void)
{
    if (GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_13) == Bit_SET) return 0;
    return 1;
}
static uint8_t IsKeyDownWakeUp(void)
{
...
{
    if (GPIO_ReadInputDataBit(GPIOG, GPIO_Pin_15) == Bit_SET) return 0;
    return 1;
}
static uint8_t IsKeyDownDown(void)
{
    if (GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_3) == Bit_SET) return 0;
    return 1;
}
static uint8_t IsKeyDownLeft(void)
{
    if (GPIO_ReadInputDataBit(GPIOG, GPIO_Pin_14) == Bit_SET) return 0;
    return 1;
}
static uint8_t IsKeyDownRight(void)
{
    if (GPIO_ReadInputDataBit(GPIOG, GPIO_Pin_13) == Bit_SET) return 0;
    return 1;
}
static uint8_t IsKeyDownOk(void)
{
    if (GPIO_ReadInputDataBit(GPIOG, GPIO_Pin_7) == Bit_SET) return 0;
    return 1;
}
static uint8_t IsKeyDownUserTamper(void)
{
    if (IsKeyDownUser() && IsKeyDownTamper()) return 1;    /* 组合键 */
    return 0;
}
...
static void NVIC_Configuration(void)
{
    ...
    /*Configuration one bit for preemption priority*/
...
static void GPIO_Configuration()
{
    ...
    /*configure PG8 extiline clock and AFIO clock*/
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC | RCC_APB2Periph_AFIO, ENABLE);

    /* configure PG8 as external interrupt */
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_13;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
    GPIO_Init(GPIOC, &GPIO_InitStructure);

    /* Connect USER Key EXTI Line to GPIOG Pin 8 */
    GPIO_EXTILineConfig(GPIO_PortSourceGPIOG, GPIO_PinSource8);

    /* Configure USER Key EXTI Line to generate an interrupt on falling edge */
    ...
    /* Clear USER Key EXTI line pending bit */
    EXTI_ClearITPendingBit(EXTI_Line13);
}
/*
*********************************************************************************************************
*   函 数 名: EXTI_PG8_Config
*   功能说明: EXTI_PG8配置。
*   形    参：无
*   返 回 值: 无
*********************************************************************************************************
*/
void rt_hw_key_EXTI_PC13_Config(void)
{
